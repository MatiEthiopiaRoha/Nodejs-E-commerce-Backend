...
async function seedUser () {
  try {
    ...
    userToBeInserted = await userDbService.findOne({ 'username':'user' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'12345',
        'isDeleted':false,
      ...
      await userDbService.update({ 'username':'user' }, userToBeInserted);
    }
    userToBeInserted = await userDbService.findOne({ 'username':'admin' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'admin',
        'isDeleted':false,
      ...
      userToBeInserted.password = await bcrypt.hash(userToBeInserted.password, 8);
      await userDbService.update({ 'username':'admin' }, userToBeInserted);
    }
